...
router.get('/performance', metricsCache, (req, res) => {
  try {
    const period = (req.query.period as '1h' | '24h' | '7d') || '1h';
    const aggregatedMetrics = metricsService.getAggregatedMetrics(period);
...
router.post('/alerts/threshold', (req, res) => {
  try {
    ...
    metricsService.addAlertThreshold({
      metric,
      operator,
      value: Number(value),
      severity,
...
router.delete('/alerts/threshold/:metric', (req, res) => {
  try {
    ...
    metricsService.removeAlertThreshold(metric);
...
router.get('/dashboard', metricsCache, (req, res) => {
  try {
    const systemMetrics = metricsService.getSystemMetrics();
    const performanceMetrics = metricsService.getAggregatedMetrics('1h');
    const alerts = metricsService.getActiveAlerts();
