...
const { imageUpload, documentUpload, multipleImages, multipleDocuments } = require('../config/upload');
...
router.post('/images', auth, multipleImages, async (req, res) => {
  try {
...
router.post('/document', auth, documentUpload.single('document'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'Nenhum arquivo enviado' });
    }
...
router.post('/documents', auth, multipleDocuments, async (req, res) => {
  try {
    if (!req.files || req.files.length === 0) {
      return res.status(400).json({ message: 'Nenhum arquivo enviado' });
    }
...
router.post('/resize-image', auth, async (req, res) => {
  try {
...
router.get('/disk-space', auth, async (req, res) => {
  try {
    const { directory } = req.query;
