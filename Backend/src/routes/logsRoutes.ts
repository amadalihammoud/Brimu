...
const logsRateLimit = securityConfig.rateLimits.api;
...
router.get('/search', (req, res) => {
  try {
    ...
    const logs = advancedLoggingService.searchLogs(filters);
...
router.get('/analytics', (req, res) => {
  try {
    ...
    const analytics = advancedLoggingService.getAnalytics(parsedTimeRange);
...
router.post('/patterns', (req, res) => {
  try {
    ...
    let processedPattern: string | RegExp = pattern;
    ...
    advancedLoggingService.addPattern({
      id,
      name,
...
router.delete('/patterns/:id', (req, res) => {
  try {
    const { id } = req.params;
    const removed = advancedLoggingService.removePattern(id);
...
router.get('/export', (req, res) => {
  try {
    ...
    const filters: any = {};
    if (level) filters.level = Array.isArray(level) ? level : [level];
    if (userId) filters.userId = userId as string;
    if (text) filters.text = text as string;
    filters.limit = parseInt(limit as string);
    ...
    const exportData = advancedLoggingService.exportLogs(format as any, filters);
...
router.post('/cleanup', (req, res) => {
  try {
    ...
    const removedCount = advancedLoggingService.cleanup(olderThan);
...
router.get('/dashboard', (req, res) => {
  try {
    ...
    const analytics = advancedLoggingService.getAnalytics(timeRange);
    const patterns = advancedLoggingService.getPatterns();

    // Logs recentes críticos
    const recentCriticalLogs = advancedLoggingService.searchLogs({
      level: ['error', 'warn'],
      timeRange,
      limit: 20
    });
...
router.get('/realtime', (req, res) => {
  try {
    ...
    // Registrar listener
    advancedLoggingService.on('log', logListener);
    ...
    req.on('close', () => {
      advancedLoggingService.removeListener('log', logListener);
