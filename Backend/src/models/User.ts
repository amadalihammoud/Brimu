...
userSchema.methods.canUpload = function(fileSize, fileCount) {
  if (!this.hasPermission('upload')) return false;
  if (fileSize > this.uploadSettings.maxFileSize) return false;
  if (fileCount > this.uploadSettings.maxFilesPerUpload) return false;
  return true;
};
