...
fileSchema.methods.canAccess = function(userId, permission = 'read') {
  ...
  return this.permissions[permission].some(perm => 
    perm.toString() === userId.toString()
  );
};

// Método estático para buscar arquivos por categoria
fileSchema.statics.findByCategory = function(category: string, options: any = {}) {
...
fileSchema.statics.findByTags = function(tags: string[], options: any = {}) {
  const query: any = { 
