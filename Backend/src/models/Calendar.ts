...
calendarEventSchema.statics.findByDateRange = function(startDate, endDate, filters = {}) {
  ...
  if (filters.status) (query as any).status = { $in: Array.isArray(filters.status) ? filters.status : [filters.status] };
  if (filters.assignedTo) (query as any)['assignedTo.user'] = filters.assignedTo;
  if (filters.client) (query as any).client = filters.client;
...
calendarEventSchema.statics.checkUserAvailability = async function(userId, startDate, endDate, excludeEventId = null) {
  ...
  if (excludeEventId) {
    (query as any)._id = { $ne: excludeEventId };
  }
