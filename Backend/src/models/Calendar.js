...
calendarEventSchema.virtual('duration').get(function() {
  if (this.startDate && this.endDate) {
    return Math.abs(this.endDate - this.startDate) / (1000 * 60); // em minutos
  }
  return 0;
});
...
calendarEventSchema.methods.generateRecurringEvents = async function() {
  ...
  const duration = endDate - baseDate;
...
calendarEventSchema.statics.findByDateRange = function(startDate, endDate, filters = {}) {
  ...
  if (filters.status) query.status = { $in: Array.isArray(filters.status) ? filters.status : [filters.status] };
  if (filters.assignedTo) query['assignedTo.user'] = filters.assignedTo;
