...
class ReportsController {
  ...
  async getDashboard(req: Request, res: Response) {
    try {
      ...
      const dashboard = await generateBusinessReport({
  ...
  async getOperationalReport(req: Request, res: Response) {
    try {
      const { period = '30d', startDate, endDate } = req.query;
  ...
  async getEquipmentReport(req: Request, res: Response) {
    try {
      const { includeUsage = 'true', includeMaintenance = 'true' } = req.query;
      
      const report = await generateEquipmentReport({
