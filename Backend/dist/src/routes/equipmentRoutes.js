...
router.get('/', cache_1.equipmentCache, (req, res) => {
    try {
        const page = (0, queryHelpers_1.parseNumberParam)(req.query.page) || 1;
        const limit = (0, queryHelpers_1.parseNumberParam)(req.query.limit) || 20;
        const skip = (page - 1) * limit;
        let filtered = equipmentStorage.filter(eq => eq.isActive);
        // Aplicar filtros
        if (req.query.status) {
            filtered = filtered.filter(eq => eq.status === (0, queryHelpers_1.parseStringParam)(req.query.status));
        }
        if (req.query.category) {
            filtered = filtered.filter(eq => eq.category === (0, queryHelpers_1.parseStringParam)(req.query.category));
        }
        if (req.query.search) {
            const search = (0, queryHelpers_1.parseStringParam)(req.query.search).toLowerCase();
...
router.get('/stats/overview', cache_1.equipmentCache, (req, res) => {
    try {
...
router.get('/:id', cache_1.equipmentCache, (req, res) => {
    try {
...
router.post('/', async (req, res) => {
    try {
        ...
        await (0, cache_1.invalidateEquipmentCache)();
        res.status(201).json({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        await (0, cache_1.invalidateEquipmentCache)(req.params.id);
        res.json({
