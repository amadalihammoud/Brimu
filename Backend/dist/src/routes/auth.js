...
router.post('/login', security_1.monitorLoginAttempts, security_1.secureTokenMiddleware, security_1.csrfProtectionMiddleware, [
    (0, express_validator_1.body)('email').isEmail().normalizeEmail().withMessage('Email inválido'),
...
router.get('/me', security_1.authenticateWithCookie, async (req, res) => {
    try {
...
router.get('/csrf-token', security_1.csrfProtectionMiddleware, (req, res) => {
    res.json({
...
router.get('/verify', security_1.authenticateWithCookie, (req, res) => {
    res.json({
        authenticated: true,
        user: req.user,
