...
orderSchema.methods.assignEquipment = async function (equipmentId, assignedBy, notes) {
    ...
    const existingAssignment = this.assignedEquipment.find(assignment => assignment.equipment.toString() === equipmentId.toString());
    if (existingAssignment) {
        throw new Error('Equipamento já está atribuído a esta ordem');
    }
    // Verificar conflitos de data se a ordem tem data agendada
    if (this.scheduling.scheduledDate) {
        const conflictingOrders = await this.constructor.checkEquipmentDateConflict(equipmentId, this.scheduling.scheduledDate, this._id);
        if (conflictingOrders.length > 0) {
            const conflictOrder = conflictingOrders[0];
            throw new Error(`Equipamento não está disponível na data ${this.scheduling.scheduledDate.toLocaleDateString('pt-BR')}. ` +
...
orderSchema.methods.updateScheduledDate = async function (newScheduledDate) {
    ...
    if (this.assignedEquipment.length > 0) {
        ...
        for (const assignment of this.assignedEquipment) {
            const conflictingOrders = await this.constructor.checkEquipmentDateConflict(assignment.equipment, newScheduledDate, this._id);
            if (conflictingOrders.length > 0) {
        ...
        if (conflicts.length > 0) {
            const conflictMessages = conflicts.map(conflict => `${conflict.equipment.name} (${conflict.equipment.code}) - conflito com ordem ${conflict.conflictingOrder._id}`);
            throw new Error(`Não é possível agendar para ${newScheduledDate.toLocaleDateString('pt-BR')}. ` +
...
orderSchema.methods.removeEquipment = function (equipmentId) {
    this.assignedEquipment = this.assignedEquipment.filter(assignment => assignment.equipment.toString() !== equipmentId.toString());
    return this.save();
};
...
orderSchema.statics.getAvailableEquipment = async function (scheduledDate, excludeOrderId = null) {
    ...
    const availableEquipment = allEquipment.filter(equipment => !occupiedEquipmentIds.has(equipment._id.toString()));
