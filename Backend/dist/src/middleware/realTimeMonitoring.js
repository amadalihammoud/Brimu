...
class RealTimeMonitoring extends events_1.EventEmitter {
    constructor() {
        super();
        this.recentRequests = [];
        this.responseTimes = [];
    ...
    updateRequestMetrics(request) {
        ...
        if (request.statusCode >= 500) {
            logger_1.logger.error('Server error request', request);
        }
    ...
    checkAlerts() {
        ...
        const recentErrors = this.errorRates.filter(error => Date.now() - error.timestamp < 5 * 60 * 1000 // últimos 5 minutos
        );
    ...
    checkSuspiciousActivity() {
        const recentRequests = this.recentRequests.filter(req => Date.now() - new Date(req.timestamp).getTime() < 5 * 60 * 1000);
    ...
    calculateAverageTimeBetweenRequests(requests) {
    ...
    cleanupOldData() {
        ...
        this.errorRates = this.errorRates.filter(error => error.timestamp > fiveMinutesAgo);
        // Limpar requests antigos (manter apenas últimas 24 horas)
        const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
        this.recentRequests = this.recentRequests.filter(req => new Date(req.timestamp).getTime() > oneDayAgo);
    ...
    getDetailedStats() {
        ...
        const recentRequests = this.recentRequests.filter(req => new Date(req.timestamp).getTime() > oneHourAgo);
        const last24hRequests = this.recentRequests.filter(req => new Date(req.timestamp).getTime() > oneDayAgo);
    ...
    resetMetrics() {
        ...
        this.alerts = [];
        this.startTime = Date.now();
        logger_1.logger.info('Monitoring metrics reset');
    }
}
// Instância singleton
