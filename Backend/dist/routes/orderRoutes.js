...
router.post('/', async (req, res) => {
    try {
        ...
        const client = await models_1.User.findById(orderData.client);
        if (!client) {
            return res.status(400).json({
                success: false,
        ...
        const service = await models_1.Service.findById(orderData.service);
        if (!service) {
            return res.status(400).json({
                success: false,
        ...
        const createdOrder = await models_1.Order.findById(order._id)
            .populate('client', 'name email phone')
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await models_1.Order.findByIdAndDelete(req.params.id);
        res.json({
            success: true,
...
router.get('/client/:clientId', async (req, res) => {
    try {
        const orders = await models_1.Order.findByClient(req.params.clientId);
        res.json({
            success: true,
            data: orders,
            count: orders.length
...
router.get('/status/:status', async (req, res) => {
    try {
        const orders = await models_1.Order.findByStatus(req.params.status);
        res.json({
            success: true,
            data: orders,
            count: orders.length
...
router.get('/stats', async (req, res) => {
    try {
        const stats = await models_1.Order.getStats();
        res.json({
            success: true,
            data: stats
        });
...
router.post('/:id/equipment', async (req, res) => {
    try {
        ...
        const equipment = await models_1.Equipment.findById(equipmentId);
        if (!equipment) {
            return res.status(404).json({
                success: false,
        ...
        // Buscar ordem atualizada
        const updatedOrder = await models_1.Order.findById(req.params.id)
            .populate('client', 'name email phone')
            .populate('service', 'name category basePrice')
            .populate('assignedTeam.user', 'name email')
            .populate('assignedEquipment.equipment', 'name code brand model status');
...
router.delete('/:id/equipment/:equipmentId', async (req, res) => {
    try {
        ...
        // Liberar equipamento
        const equipment = await models_1.Equipment.findById(req.params.equipmentId);
        if (equipment) {
...
router.get('/equipment/:equipmentId', async (req, res) => {
    try {
        const orders = await models_1.Order.findByEquipment(req.params.equipmentId);
        res.json({
            success: true,
            data: orders
        });
